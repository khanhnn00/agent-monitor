#!/usr/bin/env node
/**
 * state.cjs - machine-wide snapshot for the harness dashboard. Read-only.
 *
...
const HOST_CLAUDE = path.join(HOST_HOME, '.claude');
const DAY = 86400000;
...
const toLocal = (p) => (p && p.startsWith(HOST_CLAUDE) ? path.join(ROOT, p.slice(HOST_CLAUDE.length)) : p);
...
function lastIntent(s) {
  if (!s) return '';
  if (s.live && s.live.prompt && !NOT_INTENT.test(s.live.prompt)) return s.live.prompt;
  for (let i = s.turns.length - 1; i >= 0; i--) if (s.turns[i].prompt && !NOT_INTENT.test(s.turns[i].prompt)) return s.turns[i].prompt;
  return '';
}
...
function buildState() {
  ...
  for (const v of views) {
    const r = repos.get(v.repoKey) || { key: v.repoKey, name: path.basename(v.repoKey), path: tilde(v.repoKey), sessions: [], prompts7d: 0, skills: [], lastActive: '' };
    r.sessions.push(v);
  ...
  for (const s of ledger) {
    ...
    for (const t of s.turns) {
      if (!t.start || NOT_INTENT.test(t.prompt || '')) continue;
      if (t.start >= since14) promptLog.push({ at: t.start, repo: path.basename(s.repo || s.cwd || '?') });
      if (t.start < since) continue;
      promptTimes.push(t.start);
      activity.push({
        at: t.start, repo: path.basename(s.repo || s.cwd || '?'), session: (bySession.get(s.id) || {}).name || s.name || s.id.slice(0, 8), live: liveIds.has(s.id),
        prompt: t.prompt, model: t.model, skills: t.skills.map((k) => k.name), agents: t.agents.map((a) => a.type),
      });
      if (r) {
        r.prompts7d++;
...
function sessionDetail(id) {
  ...
  return {
    ...
    turns: s.turns.slice(-25).reverse().map((t) => ({
      n: t.n, start: t.start, end: t.end, model: t.model, prompt: t.prompt, outcome: t.outcome,
