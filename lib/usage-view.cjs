...
function sample() {
  ...
  if (a.weekly) db.weekly = { ...a.weekly, sampledAt: a.fetchedAt };
  const now = Date.now();
...
function view() {
  ...
  for (const w of windows) {
    ...
    w.othersPercent = w.minePercent == null ? null : Math.max(0, w.utilization - w.minePercent);
    delete w.atSample;
  }
  return { generatedAt: new Date(now).toISOString(), calibration: rate == null ? null : { percentPerDollar: rate, fromWindowEnd: rateFrom }, weekly: db.weekly || null, windows };
}
